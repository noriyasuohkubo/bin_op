...
cat_bin_both_models = {
    0: [
        "USDJPY_LT3_M7_LSTM1_B2_T30_I2-10-60-300_IL300-300-240-48_LU30-30-24-5_DU96-48-24-12_BNL2_BDIV0.01_201001_202210_L-RATE0.002_LT1_ADAM_d1-M1_OT-d_OD-c_IDL1_BS15360_SD0_SHU1_EL20-21-22_ub1_MN194-10",
        "USDJPY_LT4_M7_LSTM1_B2_T30_I2-10-60-300_IL300-300-240-48_LU30-30-24-5_DU96-48-24-12_BNL2_BDIV0.01_201001_202210_L-RATE0.002_LT1_ADAM_d1-M1_OT-d_OD-c_IDL1_BS15360_SD0_SHU1_EL20-21-22_ub1_MN196-31"
    ]
}
#for 0.572
#border_list_both = {-1: [0.6, 0.6], 0: [0.6, 0.6],}

border_list_both = {-1: [0.6, 0.6], 0: [0.58, 0.58],}
...
#ind_range_bases:
# [['0-0.05'], ['0-0.04'], ['0-0.06'],
# ['0.01-0.12'], ['0-0.08'], ['0-0.07'],
# ['0.04-0.16'], ['0.05-0.14'], ['0.04-0.13'],
# ['0.09-0.4'], ['0.11-0.5'], ['0.12-0.3'], ['0.12-0.3']]

#indによって対象を絞り込む場合、範囲を-区切りでリストにする
INDS = [
    ["10-satr-5", ['0-0.05']],["10-satr-15", ['0-0.04']],["10-satr-45", ['0-0.06']],
    ["60-satr-5", ['0.01-0.12']],["60-satr-15", ['0-0.08']],["60-satr-45", ['0-0.07']],
    ["300-satr-5", ['0.04-0.16']],["300-satr-15", ['0.05-0.14']],["300-satr-45", ['0.04-0.13']],
    ["1800-fsatr-5", ['0.09-0.4']],["1800-fsatr-15", ['0.11-0.5']],["1800-fsatr-45", ['0.12-0.3']],["1800-fsatr-70", ['0.12-0.3']],
]

#indの計算に必要な１番長い長さを保持
INDS_NEED_LEN = {}
INDS_NEED_LEN_MT5 = {} #MT5から取得するデータ用
...
for child in INDS:
    ...
    if "f" in idx_name:
        if (idx_term in INDS_NEED_LEN_MT5.keys()) == False :
            INDS_NEED_LEN_MT5[idx_term] = tmp_need_len
        else:
            if INDS_NEED_LEN_MT5[idx_term] < tmp_need_len:
                INDS_NEED_LEN_MT5[idx_term] = tmp_need_len
    else:
...
for tmp_a in INDS:
    ...
    if "f" in idx_name:
        #fがついている指標はmt5から取得した分足なのでチェックしない
        continue
...
def hello():
    ...
    foot_data = {}
    for foot_tmp, length in INDS_NEED_LEN_MT5.items():
        inner_dict = {}
        for name in ["close", "high", "low"]:
            tmp_closes = data[str(foot_tmp) + "_" + name]
            #必要な長さのデータが渡ってきているか確認
            if length != len(tmp_closes):
                print("error", name, length, len(tmp_closes))
                exit(1)

            inner_dict[name] = tmp_closes[:]

        foot_data[foot_tmp] = inner_dict
    ...
    if now_sec % (TRADE_DIVIDE * BET_TERM) == (tmp_divide_min * BET_TERM):
        ...
        if spr in models:
            ...
            for k,v in INDS_NEED_LEN_MT5.items():
                if (k in close_dates.keys()) == False:
                    tmp_foot_data = foot_data[k]
                    close_dates[k] = tmp_foot_data["close"]
                    high_dates[k] = tmp_foot_data["high"]
                    low_dates[k] = tmp_foot_data["low"]
