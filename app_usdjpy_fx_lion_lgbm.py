...
from app_usdjpy_fx_lion_lgbm_conf import *
...
PAST_TERM = int(get_decimal_divide(PAST_TERM_SEC, TRADE_SHIFT)) #必要とするlstmモデルの過去分予想のデータ間隔
...
def do_predict(score, closes):
    ...
    #start = time.time()
    global  prev_score
    if score - prev_score > (TRADE_SHIFT * 2):
        #もし最後のリクエストからTRADE_SHIFT * 2以上経過していたら、予想が続かなくなるので、今までの予想結果を削除
        for model in base_models:
            if model["type"] == "CATEGORY":
                predict_dict[model["no"] + "-UP"] = []
                predict_dict[model["no"] + "-SAME"] = []
            ...
            for i, p in enumerate(predict_list_rev):
                if i % PAST_TERM == 0:
                    if i == 0:
                        ...
                        base_col_dict[no + "-" + str(int(get_decimal_multi(i, TRADE_SHIFT)))] = [p]
        #print(base_col_dict)
        if len(lgbm_ds) != 0:
            # d1をlgbmモデルの特徴量とする場合
