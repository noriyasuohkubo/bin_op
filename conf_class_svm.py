...
class ConfClass():
    def __init__(self):
        ...
        self.JPY_FIX = 145  # 固定のドル円レート　BTC用
        ...
        self.PRED_TERM = 4  # 予測する間隔:DB1_TERMが2でPRED_TERMが15なら30秒後の予測をする
        ...
        self.TPSL_ADJUST_PIPS = 0.002  # スリップによる損失 testLstmFX2_rgr_limit用
        ...
        # self.FX_STOP_LOSS = 0.2 #testLstmFX2_rgr_limit用
        ...
        # DB1_TERM * DIVIDE_PREV_LENGTH 秒前のレートと予想時レートのdivideが指定divideより大きかったり小さかったら対象外とする
        # DataSequence2用
        ...
        self.DRAWDOWN_LIST = {"drawdown1": (0, -10000), "drawdown2": (-10000, -20000), "drawdown3": (-20000, -30000),
                              "drawdown4": (-30000, -40000), "drawdown5": (-40000, -50000),
                              "drawdown6": (-50000, -60000),
                              "drawdown7": (-60000, -70000), "drawdown8": (-70000, -80000),
                              "drawdown9": (-80000, -90000),
                              "drawdown9over": (-90000, -1000000), }
        self.SPREAD_LIST = {"spread0": (-1, 0), "spread1": (0, 1), "spread2": (1, 2), "spread3": (2, 3),
                            "spread4": (3, 4)
            , "spread5": (4, 5), "spread6": (5, 6), "spread7": (6, 7), "spread8": (7, 8), "spread9": (8, 9)
            , "spread10": (9, 10), "spread11": (10, 11), "spread12": (11, 12), "spread13": (12, 13),
                            "spread14": (13, 14), "spread15": (14, 15), "spread16": (15, 16),
                            "spread16Over": (16, 1000), }
        self.DIVIDE_BEF_LIST = {"divide0": (-1, 0), "divide0.1": (0, 0.1), "divide0.2": (0.1, 0.2),
                                "divide0.3": (0.2, 0.3), "divide0.4": (0.3, 0.4), "divide0.5": (0.4, 0.5),
                                "divide0.5over": (0.5, 10000), }
        self.DIVIDE_LIST = {"divide0.01": (-1, 0.01), "divide0.02": (0.01, 0.02), "divide0.03": (0.02, 0.03),
                            "divide0.04": (0.03, 0.04), "divide0.05": (0.04, 0.05)
            , "divide0.06": (0.05, 0.06), "divide0.07": (0.06, 0.07), "divide0.08": (0.07, 0.08),
                            "divide0.09": (0.08, 0.09),
                            "divide0.1": (0.09, 0.1), "divide0.2": (0.1, 0.2), "divide0.3": (0.2, 0.3),
                            "divide0.4": (0.3, 0.4), "divide0.5": (0.4, 0.5)
            ...
            , "divide1.0": (0.9, 1.0), "divide1.1": (1.0, 1.1), "divide1.2": (1.1, 1.2), "divide1.3": (1.2, 1.3),
                            "divide1.4": (1.3, 1.4)
            , "divide1.5": (1.4, 1.5), "divide1.6": (1.5, 1.6), "divide1.7": (1.6, 1.7), "divide1.8": (1.7, 1.8),
                            "divide1.9": (1.8, 1.9), "divide2.0": (1.9, 2.0), "divide2.0over": (2.0, 10000),
                            }
        self.DIVIDE_AFT_LIST = {"divide0.5": (-1, 0.5), "divide1": (0.5, 1.0), "divide2": (1, 2), "divide3": (2, 3),
                                "divide4": (3, 4)
            , "divide5": (4, 5), "divide6": (5, 6), "divide7": (6, 7), "divide8": (7, 8), "divide9": (8, 9),
                                "divide10": (9, 10)
            , "divide11": (10, 11), "divide12": (11, 12), "divide13": (12, 13), "divide13over": (13, 10000),
                                }
        self.DIVIDE_DIV_LIST = {"divide-0.5": (-1.0, -0.5), "divide-1.0": (-2.0, -1.0), "divide-2.0": (-3.0, -2.0),
                                "divide-3.0": (-4.0, -3.0), "divide-4.0": (-5.0, -4.0),
                                "divide-5.0": (-6.0, -5.0), "divide-6.0": (-7.0, -6.0),
                                "divide-7.0over": (-10000, -7.0),
                                "divide0.5": (0.5, 1.0), "divide1.0": (1.0, 2.0), "divide2.0": (2.0, 3.0),
                                "divide3.0": (3.0, 4.0), "divide4.0": (4.0, 5.0),
        ...
        #Gridsearの場合は値をリストにし、Gridsearchでない場合はスカラにする
        """
        self.SVM_PARAMS = {
            'C':[0.1, 1, 10,],
            'kernel': ['rbf'],
        ...
        self.SVM_GS_FLG = False
        self.SVM_GS_NJOBS = 1 #並列計算数。-1にすると全てのCPUコアを使用。デフォルトのNoneは1になる。
        self.SVM_GS_CV = 2 #交差検証数 2以上

        self.SVC_SCORING = "accuracy" #評価指標 see:https://helve-blog.com/posts/python/sklearn-grid-search/
        self.SVR_SCORING = "neg_mean_squared_error" #評価指標 see:https://helve-blog.com/posts/python/sklearn-grid-search/

        self.SVM_PARAMS_STR = ""
        self.SVM_GS_NJOBS_STR = ""
        self.SVM_GS_CV_STR = ""
        self.SVM_SCORING_STR = ""
        self.SVM_GS_FLG_STR = ""

        if self.METHOD == "SVC" or self.METHOD == "SVR":
            if self.SVM_GS_FLG:
                self.SVM_GS_FLG_STR = "_GS"

                for k, v in self.SVM_PARAMS.items():
                    self.SVM_PARAMS_STR = self.SVM_PARAMS_STR + "_" + k + "-" + list_to_str(v)

                self.SVM_GS_NJOBS_STR = "_NJOBS" + str(self.SVM_GS_NJOBS )
                self.SVM_GS_CV_STR = "_CV" + str(self.SVM_GS_CV)

                if self.METHOD == "SVC":
                    self.SVM_SCORING_STR = "_SCORING-" + self.SVC_SCORING
                elif self.METHOD == "SVR":
                    self.SVM_SCORING_STR = "_SCORING-" + self.SVR_SCORING
            else:
                for k, v in self.SVM_PARAMS.items():
                    self.SVM_PARAMS_STR = self.SVM_PARAMS_STR + "_" + k + "-" + str(v)
        ...
        # self.SUFFIX = "_201001_201512"
        #self.SUFFIX = "_201601_202303"
        self.SUFFIX = "_202303_202303"
    ...
    def make_file_prefix(self):
        if self.METHOD in [
            "LSTM", "BY", "LSTM2", "LSTM3", "LSTM4", "LSTM5", "LSTM6", "LSTM7", "LSTM8", "LSTM9", "LSTM10", \
                "TCN", "TCN7", "CNN", "CNN7", "SVC", "SVR"]:

            self.FILE_PREFIX = self.SYMBOL + self.LEARNING_TYPE_STR + self.ANSWER_STR + self.METHOD_STR +\
                               ...
                               "_IL" + self.INPUT_LEN_STR + self.SVM_PARAMS_STR + self.SVM_GS_NJOBS_STR + self.SVM_GS_CV_STR + self.SVM_SCORING_STR + self.FOOT_STR + \
                               self.ATR_STR + self.NORMAL_STR + self.DIVIDE_MAX_STR + self.DIVIDE_MIN_STR + \
                ...
                self.SEED) + self.DATA_SHUFFLE_STR + self.OPTIONS_STR + self.TARGET_SPREAD_STR + self.ZERO_SEC_STR + self.SUB_STR + self.HOR_STR + self.HIGHLOW_STR + self.NON_LSTM_STR + self.OANDA_ORD_STR + self.OANDA_POS_STR + \
                               self.IND_FOOT_STR + self.IND_STR + self.EXCEPT_LIST_STR + self.EXCEPT_DIVIDE_STR + self.WORKERS_STR + self.MAX_QUEUE_SIZE_STR + self.SEC_OH_LEN_FIX_STR + "_" + socket.gethostname() + "_" + self.FROM_DATA
        ...
        self.MODEL_DIR = "/app/model_svm/bin_op/" + self.FILE_PREFIX
