...
class AppUsdjpyFxPredict4Lgbm907Class():
    ...
    def __init__(self, LOGGER):
        ...
        for base_model in base_models:
            ...
            self.models[base_model["name"]] = model_tmp
            """
    ...
    def get_predict_lstm(self, score, model):
        ...
        #1秒まって予測が取得できた場合は単純に予測に時間がかかった場合なのでpredict4_907を再起動させるために例外メッセージを出す
        time.sleep(1)
        result = redis_predict_db.zrangebyscore(model["db_name"], score, score,withscores=True)
        self.LOGGER("cannot get predict:", model["db_name"], score)
        if len(result) != 0:
            raise Exception("cannot get predict. restart")
        else:
    ...
    def do_predict(self,score, closes, ):
        ...
        if get_decimal_sub(score, self.prev_score) > get_decimal_multi(LOOP_TERM, 2):
            ...
            for model in base_models:
                if model["type"] == "CATEGORY":
                    ...
                    self.predict_dict[model["no"] + "-REG"] = []
            self.LOGGER("delete predict_dict. lastscore:", self.prev_score)
        ...
        for no, predict_list in self.predict_dict.items():
            if len(predict_list) == self.PAST_LENGTH_LIST + 2:
                ...
                self.LOGGER("error!!! predict_list length not correct:", len(predict_list))
                exit(1)
    ...
    def get_predict(self, score, vals):
        ...
        if len(vals) != MAX_CLOSE_LEN:
            self.LOGGER("error!!! data length not correct:", len(vals), MAX_CLOSE_LEN)
            exit(1)

        res = self.do_predict(score, closes)
        self.LOGGER("score", score, datetime.datetime.fromtimestamp(score), "predict_take",time.perf_counter() - start_predict, "res", res)
