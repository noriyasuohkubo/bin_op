...
"""
マネパの発注履歴から勝率と平均獲得pipsを算出する
"""

#DB情報
db_name = "USDJPY_60_MONEYPARTNERS_ORDER"
db_name_history = "USDJPY_60_MONEYPARTNERS_HISTORY"

db_no = 8
#host = "192.168.1.14" #win3
host = "win5"

start = datetime(2024, 8, 1, 23)
...
for i, v in enumerate(result_data):
    ...
    start_rate_lion = float(tmps.get("start_rate_moneypartners"))
    ...
    if key in history_dict.keys():
        ...
        start_rate_lion = tmp_dict["open_rate"]
        end_rate_lion = tmp_dict["close_rate"]
        ...
        if sign == 0:
            loss_start_rate = start_rate_lion - start_rate #早く約定していれば安く買えたのに、約定が遅かった分あがってしまったレート差
            loss_end_rate = end_rate - end_rate_lion #早く約定していれば高く売れたのに、約定が遅かった分さがってしまったレート差
        elif sign == 2:
            loss_start_rate = start_rate - start_rate_lion #早く約定していれば高く買えたのに、約定が遅かった分さがってしまったレート差
            loss_end_rate = end_rate_lion - end_rate
        ...
        order_miss_list.append(position_score)

print("order miss cnt:", len(order_miss_list))
for i in order_miss_list:
