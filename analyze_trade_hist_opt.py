...
"""
the optionのCSVから取引履歴をDBに登録していく
"""
...
for FILE_NAME in FILE_NAMES:
    ...
    df = pd.read_csv(FILE, encoding="UTF-8", parse_dates=["日付", "終了時刻"], date_parser=lambda x: pd.to_datetime(x, format='="%d/%m/%Y %H:%M:%S"'))
    df["score"] = df["日付"].apply(lambda t: int(t.timestamp())) #score追加
    ...
    df = df.rename(columns={'取引銘柄': 'pair', 'レート': 'startVal', '判定レート': 'endVal', 'ペイアウト':'profit', "日付":"time"}, )
    ...
    for row in df.itertuples():
        if row.pair == PAIR:
            ...
            if len(tmp_val) == 0:
                result = "win"
                if int(row.profit.replace("¥ ", "").replace(",", "")) == 0:
                    result = "lose"
