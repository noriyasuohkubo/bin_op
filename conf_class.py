...
class ConfClass():
    def __init__(self):
        ...
        self.DTYPE = 'float32'
        self.DTYPE_STR = self.DTYPE.upper()

        self.EVAL_FLG = False  # True:学習時にevalする

        self.DEVICE = 'GPU'  # CPU or GPU
        self.DEVICE_CNT = 2  # 学習に使用するgpu deviceの数

        self.DEVICE_STR = "_" + self.DEVICE + str(self.DEVICE_CNT)

        # AMP使用の場合はTrue
        self.MIXED_PRECISION = True
        self.MIXED_PRECISION_STR = '_AMP' if self.MIXED_PRECISION else ''

        # レート取得元FX会社
        self.FROM_DATA = "DUKAS"  # OANDA

        self.JPY_FIX = 200  # 固定のGBPJPYレート　BTC用
        ...
        if self.SYMBOL == "EURUSD":
            self.PIPS = 0.00001
        elif self.SYMBOL == "GBPAUD":
            self.PIPS = 0.00001
        elif self.SYMBOL == "EURGBP":
            self.PIPS = 0.00001
        ...
        self.PRED_TERM = 30  # 予測する間隔:DB1_TERMが2でPRED_TERMが15なら30秒後の予測をする DataSequence2.pyで使用

        self.BET_SHIFT = 1   # 予測実施時の秒シフト
        self.LEARNING_SHIFT = 1 # 学習対象の秒シフト
        ...
        #self.DB_EVAL_NO = 2 #duka test
        self.DB_EVAL_NO = 1  # oanda data(GBPJPY)/oanda tick data(USDJPY)
        ...
        self.DB_SYMBOLS = [self.SYMBOL, self.SYMBOL, self.SYMBOL, self.SYMBOL, self.SYMBOL, self.SYMBOL, ]
        ...
        self.FRAGMENT_NUM = None # 参照するレート情報のインデックス.win2のDB1に登録してあるFRAGMENTSから取得する.None:設定なし
        #self.FRAGMENT_NUM = 6 # 参照するレート情報のインデックス.win2のDB1に登録してあるFRAGMENTSから取得する.None:設定なし

        self.FRAGMENTS_INPUT_LEN = 141 #特徴量の数　リストの長さ -1となる
        self.FRAGMENTS_INPUT_TYPE = "div-10000"  # 特徴量のタイプ div or div1 or sub or sub1 とmultiの値をハイフンでつなげる
        self.FRAGMENT_NUM_STR = "_FRG" + str(self.FRAGMENT_NUM) + "-" + self.FRAGMENTS_INPUT_TYPE if self.FRAGMENT_NUM != None else ""

        #分足や1時間足のデータを追加で使用する場合 空の配列:設定なし
        ...
        self.ADDITIONAL_DATA_LIST = [
            {
                ...
                'input_len':38,
                'length':"H1",
                'fragments_list':None, #DataSequence2.pyの中で取得する
             },
        ]
        """
        """
        ...
        self.ADDITIONAL_DATA_STR = ""
        for i, a in enumerate(self.ADDITIONAL_DATA_LIST):
            if i == 0:
                self.ADDITIONAL_DATA_STR = "_ADD"
            self.ADDITIONAL_DATA_STR += "-" + str(a["length"]) + "-" + str(a["fragment_num"]) + "-" + a["input_type"]

        #Datasequens2のデータに予想結果までの最安値と最高値を含める場合:True
        self.INCLUDE_HL_FLG = True
        #self.INCLUDE_HL_FLG = False

        self.TIME_DISTRIBUTED = 0  # 0なら設定なし 1:設定する
        self.TIME_DISTRIBUTED_STR = "" if self.TIME_DISTRIBUTED == 0 else "_TD"
        ...
        for idx, db in enumerate(self.FOOT_DBS):
            ...
            if idx == 0:
                ...
                self.FOOT_STR = self.FOOT_STR + "_" + str(d_term) + "-" + str(d_len) + "-" + str(
                    d_unit) + "-" + d_x + s_str
        ...
        if socket.gethostname() == 'ub3' and self.BET_TERM >= 1:
            # ub3はメモリが多いので削除しない
            self.DELETE_LEARN_FLG = False
        ...
        self.HIGHLOW_STR = "_HIGHLOW" + self.HIGHLOW_DB_CORE + "_" + str(
            self.HIGHLOW_DATA_NUM) if self.HIGHLOW_DB_CORE != "" else ""
        ...
        # リストに{"db_no":1, "inputs":[], "length":100, "unit":{48,24,12}}のような形で使用するDB番号と特徴量、データ長を列挙する
        # self.NON_LSTM_LIST = [
        # {"db_no":1, "inputs":["d1"], "length":300, "unit":{48,24,12}},
        # ]
        self.NON_LSTM_STR = ""
        if len(self.NON_LSTM_LIST) != 0:
            for i, t in enumerate(self.NON_LSTM_LIST):
                if i == 0:
                    self.NON_LSTM_STR = "_NL" + str(t["db_no"]) + "_" + list_to_str(t["inputs"]) + "_" + str(
                        t["length"])
                else:
                    self.NON_LSTM_STR = self.NON_LSTM_STR + "-" + str(t["db_no"]) + "_" + list_to_str(
                        t["inputs"]) + "_" + str(t["length"])
        ...
        if self.BET_TERM == 0.25:
            self.FX_TICK_DB = self.SYMBOL + "_" + str(self.BET_TERM) + "_0"
        ...
        # self.FX_LEVERAGE = 400  # for exness btcusd
        ...
        #self.ADJUST_PIPS = -0.001 #バイナリオプション

        #スプレッドにより獲得PIPSを調整する testLstmFX2_answer用
        self.ADJUST_PIPS_SPREAD_FLG = False

        # スプレッドMAX testLstmFX2_answer用
        self.MAX_SPREAD = 0

        self.TPSL_ADJUST_PIPS = -0.003  # スリップによる損失 testLstmFX2_rgr_limit用
        if self.SYMBOL == "EURUSD":
            self.TPSL_ADJUST_PIPS = -0.00003
        elif self.SYMBOL == "GBPAUD":
            self.TPSL_ADJUST_PIPS = -0.00003

        self.IGNORE_MINUS_SPREAD = True  # マイナススプレッドを対象外とする testLstmFX2_answer testLstmFX2_rgr_limit用

        self.BTCUSD_SPREAD_PERCENT = 0.015  # BTCUSDの取引手数料パーセント
        # self.BTCUSD_SPREAD_PERCENT = 0.0  #BTCUSDの取引手数料パーセント
        ...
        self.FX_NOT_EXT_MINUS = None  # Noneでない場合、延長判断時にこの値より利益が少ないなら延長しない。最初の予想が外れているなら、延長判断に使う予想も外れいている可能性が高い
        ...
        self.FX_MAX_POSITION_CNT = 0  # 保持可能な最大ポジション数 0なら指定なし
        #self.FX_MAX_POSITION_CNT = 2  # 保持可能な最大ポジション数 0なら指定なし
        ...
        self.TARGET_SPREAD_STR = "_TS" + list_to_str(self.TARGET_SPREAD_LISTS) if len(
            self.TARGET_SPREAD_LISTS) != 0 else ""
        ...
        #学習時に経済指標発表時間を除外する
        self.EXCEPT_IMPORTANT_INDEX_RANGE = None #None:除外しない
        #self.EXCEPT_IMPORTANT_INDEX_RANGE = 120
        self.EXCEPT_IMPORTANT_INDEX_IMPORTANCE = "importances_low" #適応する重要度 importances_high, importances_mid, importances_low
        self.EXCEPT_IMPORTANT_INDEX_STR = "" if self.EXCEPT_IMPORTANT_INDEX_RANGE == None else "_EII" + str(self.EXCEPT_IMPORTANT_INDEX_RANGE) + "-" + self.EXCEPT_IMPORTANT_INDEX_IMPORTANCE

        # EXCEPT_DIVIDE_MIN か EXCEPT_DIVIDE_MAXが0でない場合、
        # 学習時にDIVIDE_PREV_SEC 秒前のレートと予想時レートのdivideが指定divideより大きかったり小さかったら対象外とする
        # DataSequence2用
        self.DIVIDE_PREV_SEC = 300
        ...
        if self.EXCEPT_DIVIDE_MIN != 0 or self.EXCEPT_DIVIDE_MAX != 0:
            self.EXCEPT_DIVIDE_STR = "_EDS" + str(self.DIVIDE_PREV_SEC)

        if self.EXCEPT_DIVIDE_MIN != 0:
            self.EXCEPT_DIVIDE_STR = self.EXCEPT_DIVIDE_STR + "-MIN" + str(self.EXCEPT_DIVIDE_MIN)
        if self.EXCEPT_DIVIDE_MAX != 0:
            self.EXCEPT_DIVIDE_STR = self.EXCEPT_DIVIDE_STR + "-MAX" + str(self.EXCEPT_DIVIDE_MAX)
        ...
        self.DRAWDOWN_LIST = {"drawdown1": (0, -10000), "drawdown2": (-10000, -20000), "drawdown3": (-20000, -30000),"drawdown4": (-30000, -40000), "drawdown5": (-40000, -50000),"drawdown6": (-50000, -60000),
                              "drawdown7": (-60000, -70000), "drawdown8": (-70000, -80000),"drawdown9": (-80000, -90000),"drawdown9over": (-90000, -1000000), }
        self.SPREAD_LIST = {"spread0": (-1, 0), "spread1": (0, 1), "spread2": (1, 2), "spread3": (2, 3),"spread4": (3, 4), "spread5": (4, 5), "spread6": (5, 6), "spread7": (6, 7),"spread8": (7, 8), "spread9": (8, 9)
        ...
        self.DIVIDE_LIST = {"divide0.01": (-1, 0.01), "divide0.02": (0.01, 0.02), "divide0.03": (0.02, 0.03),
                            "divide0.04": (0.03, 0.04), "divide0.05": (0.04, 0.05), "divide0.06": (0.05, 0.06), "divide0.07": (0.06, 0.07), "divide0.08": (0.07, 0.08),"divide0.09": (0.08, 0.09),
                            "divide0.1": (0.09, 0.1), "divide0.2": (0.1, 0.2), "divide0.3": (0.2, 0.3),"divide0.4": (0.3, 0.4), "divide0.5": (0.4, 0.5), "divide0.6": (0.5, 0.6), "divide0.7": (0.6, 0.7), "divide0.8": (0.7, 0.8), "divide0.9": (0.8, 0.9)
                            , "divide1.0": (0.9, 1.0), "divide1.1": (1.0, 1.1), "divide1.2": (1.1, 1.2), "divide1.3": (1.2, 1.3),
                            "divide1.4": (1.3, 1.4), "divide1.5": (1.4, 1.5), "divide1.6": (1.5, 1.6), "divide1.7": (1.6, 1.7), "divide1.8": (1.7, 1.8),
                            "divide1.9": (1.8, 1.9), "divide2.0": (1.9, 2.0), "divide2.0over": (2.0, 10000),
                            }
        ...
        self.DIVIDE_DIV_LIST = {"divide-0.5": (-1.0, -0.5), "divide-1.0": (-2.0, -1.0), "divide-2.0": (-3.0, -2.0),"divide-3.0": (-4.0, -3.0), "divide-4.0": (-5.0, -4.0),
                                "divide-5.0": (-6.0, -5.0), "divide-6.0": (-7.0, -6.0),"divide-7.0over": (-10000, -7.0),"divide0.5": (0.5, 1.0), "divide1.0": (1.0, 2.0), "divide2.0": (2.0, 3.0),
                                "divide3.0": (3.0, 4.0), "divide4.0": (4.0, 5.0),"divide5.0": (5.0, 6.0), "divide6.0": (6.0, 7.0),"divide7.0over": (7.0, 10000),
                                }
        # 学習方法
        self.METHODS = {
        ...
        #LSTMのreturn_sequences
        self.RETURN_SEQ = False
        self.RETURN_SEQ_STR = "_RS" if "LSTM" in self.METHOD and self.RETURN_SEQ else ""

        # LSTM7の時に、秒、分、時データにdenseをかける場合
        # self.DENSE_UNIT_LSTM7 =[]
        self.DENSE_UNIT_LSTM7 = [
            24,
            12,
            6,
        ]
        self.DENSE_UNIT_LSTM7_STR = "_" + list_to_str(self.DENSE_UNIT_LSTM7) if self.METHOD == "LSTM7" else ""
        ...
        # LSTM-ATTENTION, LSTM-ATTENTION-LSTM,
        # CNN-LSTM, SimpleRNN
        self.LSTM_TYPE = "LSTM"

        self.LSTM_LAYER_NUM = 1  # LSTM層の数

        # LSTM-ATTENTIONの場合の設定
        self.LSTM_ATTENTION_CONF = {
            'USE_STATE': True,
            'FEATURE_DENSE': [36, 18, 9],
            'ATTENTION_CNT': 1,  # 幾つattentnionを使うか
            'LSTM_UNIT': 10,  # lstmのunit数 LSTM-ATTENTION-LSTM専用
        }

        self.LSTM_ATTENTION_CONF_STR = ""
        if self.LSTM_TYPE == "LSTM-ATTENTION" or self.LSTM_TYPE == "LSTM-ATTENTION-LSTM":
            for i, (k, v) in enumerate(self.LSTM_ATTENTION_CONF.items()):
                if i == 0:
                    self.LSTM_ATTENTION_CONF_STR = "_LAC"
                if k == 'USE_STATE' and v == True:
                    self.LSTM_ATTENTION_CONF_STR = self.LSTM_ATTENTION_CONF_STR + "-" + 'US'
                elif k == 'FEATURE_DENSE':
                    self.LSTM_ATTENTION_CONF_STR = self.LSTM_ATTENTION_CONF_STR + "-" + 'FD' + list_to_str(v)
                elif k == 'ATTENTION_CNT':
                    self.LSTM_ATTENTION_CONF_STR = self.LSTM_ATTENTION_CONF_STR + "-" + 'AC' + str(v)
                else:
                    if self.LSTM_TYPE == "LSTM-ATTENTION-LSTM":
                        self.LSTM_ATTENTION_CONF_STR = self.LSTM_ATTENTION_CONF_STR + "-" + k + str(v)
        ...
        self.CNN_UNIT_NUM = 10
        self.CNN_UNIT_NUM_STR = ""

        self.CNN_KERNEL_SIZE = 20
        ...
        elif self.LSTM_TYPE in ["CNN-LSTM" ]:
            self.CNN_UNIT_NUM_STR = "_CNNUN" + str(self.CNN_UNIT_NUM) + "-" + str(self.CNN_KERNEL_SIZE)
        ...
        # self.BATCH_SIZE = 1024 * 2.5
        self.BATCH_SIZE = 1024 * 30
        #self.BATCH_SIZE = 1024 * 30 * 5
        # self.BATCH_SIZE = 1024 * 3 #テスト時にエラーでた場合はバッチサイズさげる

        self.DROP_LAST = True
        self.DL_STR = "_DL" + str(self.DROP_LAST)
        ...
        self.LEARNING_TYPES = {
            "CATEGORY": 1, "CATEGORY_BIN": 2, "CATEGORY_BIN_UP": 3, "CATEGORY_BIN_DW": 4, "CATEGORY_BIN_BOTH": 5,
            "CATEGORY_BIN_FOUR": 6,
            "REGRESSION_SIGMA": 7, "REGRESSION": 8, "REGRESSION_UP": 9, "REGRESSION_DW": 10, "REGRESSION_OCOPS": 11,
            "CATEGORY_OCOPS": 12, "CATEGORY_BIN_UP_IFD": 13, "CATEGORY_BIN_DW_IFD": 14, "CATEGORY_BIN_UP_IFO": 15,
            "CATEGORY_BIN_DW_IFO": 16,
            "CATEGORY_BIN_UP_IFDSF": 17, "CATEGORY_BIN_DW_IFDSF": 18, "CATEGORY_BIN_UP_TP": 19,
            "CATEGORY_BIN_DW_TP": 20,
            "CATEGORY_BIN_UP_OCO": 21, "CATEGORY_BIN_DW_OCO": 22, "CATEGORY_BIN_UP_OCOA": 23,
            "CATEGORY_BIN_DW_OCOA": 24,
        }
        ...
        if self.LEARNING_TYPE_STOPLOSS:
            ...
            if (self.LEARNING_TYPE in ["CATEGORY", "CATEGORY_BIN_UP", "CATEGORY_BIN_DW"]) == False:
                print("LEARNING_TYPE_STOPLOSS must be in CATEGORY or CATEGORY_BIN_UP or CATEGORY_BIN_DW")
                exit(1)
        ...
        #self.INPUT_DATAS = ["c",]
        self.INPUT_DATAS = ["d1",]
        #self.INPUT_DATAS = ["d1","d1",]
        #self.INPUT_DATAS = ["d1","d1","d1",]
        #self.INPUT_DATAS = ["d1","d1","d1","d1","d1",]
        # self.INPUT_DATAS = ["d1", "d1", "d1","d1","d1", ]
        # self.INPUT_DATAS = ["std~d1", "std~d1", "std~d1", ]
        # self.INPUT_DATAS = ["d1","d1_ehd1-1_eld1-1", ]
        # self.INPUT_DATAS = ["d1","d1_ehd1-1_eld1-1","d1_ehd1-1_eld1-1", "d1_ehd1-1_eld1-1", ]
        # self.INPUT_DATAS = ["d1","d1","d1_ehd1-1_eld1-1", ]
        #self.INPUT_DATAS = ["d1", "d1_ehd1-1_eld1-1", "d1_ehd1-1_eld1-1", ]
        # self.INPUT_DATAS = ["sma-5",]

        # self.INPUT_DATAS = ["d1","d1_md1-1","d1_md1-1",]
        # self.INPUT_DATAS = ["d1","d1_md1-1_ehd1-1_eld1-1","d1_md1-1_ehd1-1_eld1-1",]
        ...
        self.OUTPUT_TYPE_STR = "_OT-" + self.OUTPUT_TYPE if self.OUTPUT_MULTI == 1 else "_OT-" + self.OUTPUT_TYPE + "-M" + str(
            self.OUTPUT_MULTI)
        ...
        # 特徴量がstd~d1の場合の最大値 これを超えた場合はこの最大値に変換する
        self.STD_D1_MAX = 0  # 0なら設定なし
        self.STD_D1_STR = "_STD_D1_MAX" + str(self.STD_D1_MAX) if self.STD_D1_MAX != 0 else ""
        ...
        # DataSequence2で正解のYを複数作成する場合は追加する
        # self.RETURN_Y_LIST = []
        self.RETURN_Y_LIST = [
            #self.LEARNING_TYPE + "-" + "d" + "-" + str(0.1),
            self.LEARNING_TYPE + "-" + "d" + "-" + str(0.5),
            self.LEARNING_TYPE + "-" + "d" + "-" + str(1.0),
            #"REGRESSION" + "-" + "d" + "-",
        ]
        ...
        self.BORDER_STR = ""
        if self.LEARNING_TYPE == "CATEGORY" or self.LEARNING_TYPE == "CATEGORY_BIN_BOTH" or self.LEARNING_TYPE == "CATEGORY_BIN" or self.LEARNING_TYPE == "CATEGORY_BIN_UP" or self.LEARNING_TYPE == "CATEGORY_BIN_DW":
            if self.OUTPUT_TYPE == "sub":
                self.BORDER_STR = "_SPREAD" + str(self.SPREAD)
                self.GET_Y_STR = self.LEARNING_TYPE + "-" + self.OUTPUT_TYPE + "-" + str(self.SPREAD)
                self.RETURN_Y_LIST.insert(0, self.GET_Y_STR)

            elif self.OUTPUT_TYPE == "d":
                self.BORDER_STR = "_BDIV" + str(self.BORDER_DIV)
                self.GET_Y_STR = self.LEARNING_TYPE + "-" + self.OUTPUT_TYPE + "-" + str(self.BORDER_DIV)
                self.RETURN_Y_LIST.insert(0, self.GET_Y_STR)

        elif self.LEARNING_TYPE == "REGRESSION":
            self.GET_Y_STR = self.LEARNING_TYPE + "-" + self.OUTPUT_TYPE + "-"
            self.RETURN_Y_LIST.insert(0, self.GET_Y_STR)

        self.RETURN_Y_LIST_STR = "_RETY:" + list_to_str(self.RETURN_Y_LIST, spl=":")
        ...
        if self.LEARNING_TYPE in ["CATEGORY", "CATEGORY_BIN_BOTH", "CATEGORY_OCOPS"]:
            self.OUTPUT = 3
                                    ...
                                    "CATEGORY_BIN_UP_IFDSF", "CATEGORY_BIN_DW_IFDSF", "CATEGORY_BIN_UP_TP",
                                    "CATEGORY_BIN_DW_TP",
                                    "CATEGORY_BIN_UP_OCO", "CATEGORY_BIN_DW_OCO", "CATEGORY_BIN_UP_OCOA",
                                    "CATEGORY_BIN_DW_OCOA"]:
            self.OUTPUT = 2
        ...
        # C-ENTROPY-FOCALの場合のパラメータ
        self.C_ENTROPY_FOCAL_AlPHA = [1.5, 1, 1.5]
        self.C_ENTROPY_FOCAL_GAMMA = 2

        if self.MIXTURE_NORMAL:
            # MIXTURE_NORMALの場合強制的にNLL
            self.LOSS_TYPE = "NLL"

        if self.LOSS_TYPE in ["FXMSE", "FXMSE2", "FXMSE3"]:
            self.LOSS_STR = "_" + str(FX_LOSS_PNALTY)
        elif self.LOSS_TYPE in ["MSEC"]:
            self.LOSS_STR = str(MSE_PENALTY)
        elif self.LOSS_TYPE in ["IE"]:
            self.LOSS_STR = str(INSENSITIVE_BORDER)
        elif self.LOSS_TYPE in ["C-ENTROPY-FOCAL"]:
            self.LOSS_STR = list_to_str(self.C_ENTROPY_FOCAL_AlPHA) + "_" + str(self.C_ENTROPY_FOCAL_GAMMA)
        else:
            self.LOSS_STR = ""
        ...
        self.LSTM_ACTIVATION = ""  # LSTM結果に活性化をかけない場合は空文字指定
        # self.LSTM_ACTIVATION = "gelu"
        self.LSTM_ACTIVATION_STR = "_LA" + str(
            self.ACTIVATION_TYPES[self.LSTM_ACTIVATION]) if self.LSTM_ACTIVATION != "" else ""

        # optimizer:ADAM, ADABOUND, AMSBOUND,
        # AdamW,RectifiedAdam,LazyAdam, Adamax, Nadam
        # SGD
        self.OPT = "Adam"
        self.OPT_OPT = 0.0
        self.OPT_STR = "" if self.OPT_OPT == 0.0 else "-" + str(self.OPT_OPT)

        self.INIT_STR = "_GU-GU-GU"  # 初期値の設定(R_I,D_I,O_I)によってモデル名のsuffixを変える

        self.SUFFIX = ""

        #self.TRAIN_START_DT = "20040101"  # 学習開始年月日
        self.TRAIN_START_DT = "20100101"  # 学習開始年月日
        #self.TRAIN_START_DT = "20160101"  # 学習開始年月日
        #self.TRAIN_START_DT = "20200101"  # 学習開始年月日
        # self.TRAIN_START_DT = "20230325"  # 学習開始年月日
        #self.TRAIN_END_DT = "20240101"
        self.TRAIN_END_DT = "20241201"
        #self.TRAIN_END_DT = "20250831"

        #self.EVAL_START_DT = "20220101"
        #self.EVAL_START_DT = "20240101"
        self.EVAL_END_DT = "20241201"
        #self.EVAL_END_DT = "20250831"

        #self.EVAL_START_DT = "20250901"
        self.EVAL_END_DT = "20260502"

        if self.EVAL_FLG:
            self.TRAIN_EVAL_TERM_STR = "_" + self.TRAIN_START_DT + "-" + self.TRAIN_END_DT + "_" + self.EVAL_START_DT + "-" + self.EVAL_END_DT
        else:
            self.TRAIN_EVAL_TERM_STR = "_" + self.TRAIN_START_DT + "-" + self.TRAIN_END_DT

        # 学習時のデータを既にDataSequence2_make_pickle.pyで作成してあるpickleで取得する場合の設定
        self.DATA_SEQUENCE_FROM_PICKLE_CONF = []  # 設定なしの場合:空の辞書

        self.DATA_SEQUENCE_FROM_PICKLE_CONF = [
            #{
            #    "score": "311",
            #    "save_dir_path": "/nvme1/dataSequence2/USDJPY/DS2F311-0",
            #},
        ]

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_ON_MEMORY = [
            {
                "score": "573",
                "save_dir_path": "/nvme1/dataSequence2/USDJPY/DS2F573-0",
            },
            {
                "score": "637",
                "save_dir_path": "/nvme1/dataSequence2/USDJPY/DS2F637-0",
            },
        ]

        # 学習時のデータを既にDataSequence2_make_pickle.pyで作成してあるDataSequence2で取得する場合の設定
        self.DATA_SEQUENCE_FROM_PICKLE_CONF_RAW = {
            # "score":"102",
            # "save_dir_path": "/nvme1/dataSequence2/USDJPY/DS2F102-0",
        }

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_EVAL = [
            #{
                #"score": "444",
                #"save_dir_path": "/nvme2/dataSequence2/USDJPY/DS2F444-0",
            #},
        ]

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST = {

        }

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY = {
            #"score": "444",
            #"save_dir_path": "/nvme2/dataSequence2/USDJPY/DS2F444-0",
        }

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_EVAL_ON_MEMORY = [
            #{
            #    "score": "450",
            #    "save_dir_path": "/nvme2/dataSequence2/USDJPY/DS2F450-0",
            #}
        ]

        # 学習時のデータを既にDataSequence2_make_pickle.pyで作成してあるDataSequence2で取得する場合の設定
        self.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_RAW = [
        ...
        self.TRANSFER_CONF_STR = "" if len(self.TRANSFER_CONF) == 0 else "_TC-" + self.TRANSFER_CONF["MN"]

        self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR = ""
        if len(self.DATA_SEQUENCE_FROM_PICKLE_CONF) != 0:
            scores = []
            for tmp_c in self.DATA_SEQUENCE_FROM_PICKLE_CONF:
                tmp_symbol = tmp_c["save_dir_path"].split("/")[3]
                scores.append(tmp_symbol + "-" + tmp_c["score"])
            score_str = list_to_str(scores)

            if self.EVAL_FLG:
                scores = []
                for tmp_c in self.DATA_SEQUENCE_FROM_PICKLE_CONF_EVAL:
                    scores.append(tmp_c["score"])
                score_str_eval = list_to_str(scores)

                self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR = "_DSFPC-" + score_str + "_" + score_str_eval
            else:
                self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR = "_DSFPC-" + score_str

        elif len(self.DATA_SEQUENCE_FROM_PICKLE_CONF_ON_MEMORY) != 0:
            scores = []
            for tmp_c in self.DATA_SEQUENCE_FROM_PICKLE_CONF_ON_MEMORY:
                tmp_symbol = tmp_c["save_dir_path"].split("/")[3]
                scores.append(tmp_symbol + "-" + tmp_c["score"])
            score_str = list_to_str(scores)
        ...
        elif len(self.DATA_SEQUENCE_FROM_PICKLE_CONF_RAW) != 0:
            scores = []
            scores.append(self.DATA_SEQUENCE_FROM_PICKLE_CONF_RAW["score"])
            score_str = list_to_str(scores)
            ...
            if self.EVAL_FLG:
                ...
                self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR = "_DSFPCR" + score_str + "_" + score_str_eval
            else:
                self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR = "_DSFPCR" + score_str
        ...
        if self.METHOD == "LSTM8":
            self.SUFFIX += "_VOL-" + str(
                int(Decimal(str(self.DB_VOLUME_TERM)) * Decimal(str(self.DB_VOLUME_INPUT_LEN))))
        ...
        self.MODEL_DIR_PARENT = '/app/model/bin_op/'
        self.HISTORY_DIR_PARENT = "/app/history/bin_op/"

        # 既存モデルをロードして続きを学習する場合に設定する
        self.LOAD_CONF_DICT = {}  # 設定なしの場合は空の辞書
        """
        self.LOAD_CONF_DICT = {
            'MN':1829,
            'LOAD_EPOCH':588,
        ...
        """
        self.LOAD_CONF_DICT_STR = ""
        for i, (k, v) in enumerate(self.LOAD_CONF_DICT.items()):
            if i == 0:
                self.LOAD_CONF_DICT_STR = "_LCD"
            self.LOAD_CONF_DICT_STR = self.LOAD_CONF_DICT_STR + "-" + k + str(v)
    ...
    def make_file_prefix(self):
        if self.METHOD in [
            ...
            self.FILE_PREFIX = self.SYMBOL + '_' + self.DTYPE_STR + self.DEVICE_STR + self.MIXED_PRECISION_STR + self.DL_STR + self.LEARNING_TYPE_STR + self.ANSWER_STR + \
                               self.LSTM_ATTENTION_CONF_STR + self.METHOD_STR + "_" + self.LSTM_TYPE + str(
                self.LSTM_LAYER_NUM) + self.TCN_ARG_STR + self.RETURN_SEQ_STR + self.WINDOW_SIZE_STR + self.MIXTURE_NORMAL_STR + \
                               "_B" + str(self.BET_TERM) + "_BS" + str(self.BET_SHIFT) + "_LS" + str(self.LEARNING_SHIFT) + "_T" + str(
                self.TERM) + self.START_TERM_STR + self.END_TERM_STR + \
                               self.DB_SYMBOLS_STR + self.DB1_NOT_LEARN_STR + "_I" + self.DB_TERM_STR + \
                               "_IL" + self.INPUT_LEN_STR + "_LU" + self.LSTM_UNIT_STR + "_DU" + self.DENSE_UNIT_STR + self.FRAGMENT_NUM_STR + self.ADDITIONAL_DATA_STR + self.TIME_DISTRIBUTED_STR + self.DENSE_UNIT_LSTM7_STR + self.FOOT_STR + \
                               ...
                               self.L_K_STR + self.L_R_STR + self.L_D_STR + self.ATR_STR + self.NORMAL_STR + self.STD_D1_STR + self.DIVIDE_MAX_STR + self.DIVIDE_MIN_STR + \
                               self.BORDER_STR + self.SUFFIX + self.LEARNING_RATE_STR + self.LOSS_TYPE_STR + self.LOSS_STR + "_" + self.OPT + self.OPT_STR + self.LSTM_ACTIVATION_STR + self.DENSE_ACTIVATION_STR + \
                               ...
                               "_BS" + str(self.BATCH_SIZE) + "_SD" + str(
                self.SEED) + self.DATA_SHUFFLE_STR + self.OPTIONS_STR + self.TARGET_SPREAD_STR + self.ZERO_SEC_STR + self.SUB_STR + self.HOR_STR + self.HIGHLOW_STR + self.NON_LSTM_STR + self.OANDA_ORD_STR + self.OANDA_POS_STR + \
                               self.IND_FOOT_STR + self.IND_STR + self.EXCEPT_LIST_STR + self.EXCEPT_IMPORTANT_INDEX_STR + self.EXCEPT_DIVIDE_STR + self.EXCEPT_DIVIDE_MIN_AFTER_STR + self.WORKERS_STR + self.MAX_QUEUE_SIZE_STR + self.SEC_OH_LEN_FIX_STR + self.DATA_SEQUENCE_FROM_PICKLE_CONF_STR + self.TRANSFER_CONF_STR + self.LOAD_CONF_DICT_STR + self.TRAIN_EVAL_TERM_STR + self.RETURN_Y_LIST_STR + "_" + socket.gethostname() + "_" + self.FROM_DATA

        elif self.METHOD == "NORMAL":
            self.FILE_PREFIX = self.SYMBOL + self.LEARNING_TYPE_STR + self.METHOD_STR + "_BET" + str(
                self.BET_TERM) + "_TERM" + str(self.TERM) + \
                               ...
                               self.BORDER_STR + self.SUFFIX + self.LEARNING_RATE_STR + "_LOSS-" + self.LOSS_TYPE + self.LOSS_STR + "_" + self.OPT  + self.OPT_STR+ self.INPUT_DATA_STR + self.OUTPUT_TYPE_STR + self.OUTPUT_DATA_STR + "_BS" + \
                               str(self.BATCH_SIZE) + "_SEED" + str(
                self.SEED) + "_" + self.DATA_SHUFFLE + self.OPTIONS_STR + self.SUB_STR + self.EXCEPT_LIST_STR + "_" + socket.gethostname()
        ...
        if len(result) == 0:
            ...
            for line in result:
                ...
                if tmp_name == self.FILE_PREFIX:
                    # 同じモデルがないが確認
                    print("The Model Already Exists!!!", score)
                    exit(1)

            self.MN = "MN" + str(newest_no)  # モデルに番号をつける
            self.FILE_PREFIX = self.FILE_PREFIX + "_" + self.MN
    ...
    def get_fx_position(self, rate):
        if self.FX_FIX_POSITION == 0:
            return self.FX_LEVERAGE * self.FX_FUND / rate / self.FX_MAX_POSITION_CNT
    ...
    def get_fx_position_jpy(self, rate, jpy):
        if self.FX_FIX_POSITION == 0:
            return self.FX_LEVERAGE * self.FX_FUND / jpy / rate / self.FX_MAX_POSITION_CNT
