...
class DataSequence2_from_pickle_test_on_memory(Sequence):

    def __init__(self, c, eval_flg=False ):

        self.c = c
        self.epoch_cnt = 0
        self.get_time_list = []
        score = c.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY["score"]
        ...
        # dir_pathが合っているか確認
        if (c.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY["score"] in self.c.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY["save_dir_path"]) == False:
            print("save_dir_path not correct", self.c.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY["save_dir_path"])
            exit(1)

        self.save_dir_path = self.c.DATA_SEQUENCE_FROM_PICKLE_CONF_TEST_ON_MEMORY["save_dir_path"]
        # ディレクトリが既に存在しているか確認
        ...
        for j in range(self.steps_per_epoch):
            tmp_pickle_path = self.save_dir_path + "/BF" + str(j)
    ...
    def __getitem__(self, idx):
        ...
        retX = self.memory[self.step_list[idx]]
