...
for past_term in past_terms:
    result_data = redis_db.zrangebyscore(db_name, get_decimal_sub(get_decimal_sub(key_index, past_term), get_decimal_multi(MAX_CLOSE_LEN, AI_MODEL_TERM)), get_decimal_sub(get_decimal_sub(key_index, past_term), AI_MODEL_TERM), withscores=True)
    ...
    for model in base_models:
        ...
        x = get_x(False, get_decimal_sub(key_index, past_term), model["data_length"], model["input_datas"], model["input_separate_flg"], model["method"], closes=closes)
        # print(x)
        t1 = time.time()
...
#最新の予想
result_data = redis_db.zrangebyscore(db_name, get_decimal_sub(key_index, get_decimal_multi(MAX_CLOSE_LEN, AI_MODEL_TERM)), get_decimal_sub(key_index, AI_MODEL_TERM), withscores=True)
#print(len(result_data))
closes = []
