...
from app_usdjpy_fx_thinkm_lgbm_conf import *
...
def do_predict(score, closes):
    ...
    #start = time.time()
    global  prev_score
    if score - prev_score > (LOOP_TERM * 2):
        ...
        for model in base_models:
            if model["type"] == "CATEGORY":
                ...
                predict_dict[model["no"] + "-REG"] = []
        print("delete predict_dict lastscore:", prev_score, " nowscore:", score)
...
def hello():
    ...
    print(res)
    print("predict_take:", time.perf_counter() - start_take)
